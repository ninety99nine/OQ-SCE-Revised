<template>
    ...
    <div :class="['border-l border-dashed']">
        ...
        <div class="flex items-center pl-1 -ml-1 cursor-pointer bg-white" @click="toggle()">
            ...
            <span v-if="!active" class="text-blue-300 text-xs ml-2"> x {{ Object.keys(modelValue).length }}</span>

        </div>

        <SlideUpDown v-model="active" :duration="duration">

            <KeyValueInput v-model="modelValue" :label="label" :level="level" class="mt-4" />
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateValue(event) {
                this.$emit('update:modelValue', event /* event.target.value */);
            }
        },
        created() {
