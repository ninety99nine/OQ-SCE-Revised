    ...
    <div class="h-48 overflow-y-auto border bg-gray-50 rounded-md p-2 resize-y">
        ...
        <draggable
            ...
            <template #item="{element, index}">
                <ScreenMenu :screen="element" :index="index" :key="element.id" class="draggble-screen"></ScreenMenu>
            </template>
            <template v-if="filteredScreens.length === 0" #footer>
                <svg xmlns="http://www.w3.org/2000/svg" class="h-40 w-40 text-blue-100 m-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<script>
    ...
    export default {
        components: { draggable, ScreenMenu },
        data() {
            return {
