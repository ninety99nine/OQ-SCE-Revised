    ...
    <div>
        ...
        <template v-if="form.event_data.response.selected_type == 'manual'">
            ...
            <el-tabs v-model="activeTab" type="card" closable>
                <el-tab-pane v-for="(statusHandle, index) in form.event_data.response.manual.response_status_handles" :key="index">

                    <template v-slot:label>
                        <span class="text-gray-800 ">{{ statusHandle.status }}</span>
                    </template>
                    ...
                    <div class="bg-gray-50 border rounded-md p-4">
                        ...
                        <VariableInput v-model="statusHandle.reference_name" label="Response Name" :error="form.errors.response_reference_name" @onSetError="form.setError('response_reference_name', $event)" @onClearError="form.clearErrors('response_reference_name')" class="mb-6"></VariableInput>
...
<script>
    ...
    export default {
        props: ['form'],
        components: { PrimaryAlert, VariableInput, DefaultSelect, ResponseStatusAttributes, TextOrCodeEditor, CreateOrUpdateStatusCodeModal },
        data(){
            return {
                activeTab: 0,
                apiResponseOptions: [
