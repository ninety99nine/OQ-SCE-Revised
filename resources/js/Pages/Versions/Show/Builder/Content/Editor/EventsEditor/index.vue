<template>
    ...
    <div :class="{ 'p-4 rounded-lg bg-gray-50 shadow-md border': highlight }">
                ...
                <Transition name="fade">
                    ...
                    <div v-if="isShowingEvents" class="flex items-center">
                        ...
                        <template v-if="totalEvents">
                            <CreateOrUpdateEventModal v-if="events.hasOwnProperty('collection')" :events="events.collection" mode="create" class="mr-4"></CreateOrUpdateEventModal>
                            <CreateOrUpdateEventModal v-else :events="events" mode="create" class="mr-4"></CreateOrUpdateEventModal>
                        </template>
                    </div>
        ...
        <SlideUpDown v-model="isShowingEvents" :duration="300">
            ...
            <template v-if="events.hasOwnProperty('conditional')">

                <!-- Conditional Events Code Editor -->
                <SwitchAndCodeEditor
                    v-model="events.conditional" max_height="200px" class="mb-4"
            ...
            <!--
                ...
                we must conditionally check for either case to access the nested events.
            -->
            <EventMenus v-if="events.hasOwnProperty('collection')" v-model="events.collection" :searchTerm="searchTerm" :class="['transition-all duration-1000', isShowingEvents ? 'mb-0': 'mb-8']"></EventMenus>
            <EventMenus v-else v-model="events" :searchTerm="searchTerm" :class="['transition-all duration-1000', isShowingEvents ? 'mb-0': 'mb-8']"></EventMenus>
...
export default {
    ...
    computed: {
        ...
        totalEvents() {
            return (this.events.hasOwnProperty('collection') ? this.events.collection : this.events).length
        }
    },
    methods: {
        pasteEvent() {
            ...
            if( event !== null ) {
                ...
                if( this.events.hasOwnProperty('collection') ) {

                    this.events.collection.push(event);
