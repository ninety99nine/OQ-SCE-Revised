<template>
    ...
    <DefaultModal
        ...
        <!-- Modal Title -->
        <template v-slot:title>Delete Status Code</template>

        <!-- Modal Content (If we are deleting single status code) -->
        <p v-if="mode == 'single' && statusCode" class="text-sm text-gray-500 mb-5">Are you sure you want to delete the <span class="text-blue-500 font-bold">{{ statusCode.name }}</span> status code. After this status code is deleted you cannot recover it again.</p>
