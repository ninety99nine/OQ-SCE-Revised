    ...
    <div>
        ...
        <Transition name="fade" mode="out-in" appear>
            <div v-if="matchingGlobalVariableNames.length && isGlobal == false && allowGlobalVariables && hintGlobalVariables" class="space-x-2 mt-2 overflow-hidden">
                <OrangeBadge v-for="(name, index) in matchingGlobalVariableNames" :key="index" class="cursor-pointer hover:animate-pulse" @click="modelValue = name">{{ name }}</OrangeBadge>
            </div>
        </Transition>
    ...
    export default {
        ...
        computed: {
            matchingGlobalVariableNames() {
                if( this.modelValue ) return this.useVersionBuilder.searchGlobalVariables(this.modelValue, false).map((globalVariable) => globalVariable.name);
                return [];
            },
            isGlobal() {
